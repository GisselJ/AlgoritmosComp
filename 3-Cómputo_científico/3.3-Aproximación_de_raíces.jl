...
#> [frontmatter]
#> title = "3.3-Aproximacion-de-raices-G"
...
# ╔═╡ b10dc7b9-b273-4e0f-99de-983c499e2f95
using Plots, PlutoUI
...
_Vemos que para obtener la iteración 

"""

# ╔═╡ 0a32636b-619c-4bef-bf6a-75c33ecb8a9c
$x_n$, se retoma $x_{n-1}$, de manera que, recorriendo el "-1" de observación, vemos que se cumple:_

$x_{i+1} =x_i-\frac{f(x_i)}{f\prima(x_i)}$ 
...
function newton(f, f′, x0, n)
	#inicializamos un valor que irá variando conforme la iteración
	x= x0 #condición inicial
	for j in 1:n #marcamos el número de iteraciones
		x = x - (f(x0)/f′(x0)) #despejamos de la aproximación de delta, tomando en cuenta que delta= x-x0
	end
	return x
end
...
function newtonRecursiva(f, f′, x0, n)
	...
	x= x0 #condición inicial
	if n == 0
		return x0 #no se aproxima, se reentrega el valor introducido
	else
		x = x- (f(x0)/f′(x0)) #despejamos de la aproximación de delta, tomando en cuenta que delta= x-x0
		return newtonRecursiva(f, f′, x0, n-1) #usamos recursión, n-1 está dentro de la función para n
	end
end
...
function newtonDistancia(f, f′, x0, ε)
    xa = x0 #definimos la primera aproximación, la base, con x0 dado
    xi = xa - f(xa) / f′(xa) #xi: x a iterar, para seguir aproximando, depende de la anterior, que variará dentro del ciclo, hasta éste punto es fija, y es la primera aproximación
    
    while abs(xi - xa) > ε #abs: distancia 
		# si se cumple, entonces seguimos aproximando
        xa = xi #pasamos la última x calculada a ser la anterior 
        xi = xi - (f(xi) / f′(xi)) #siguiente aproximació
    end
	if ε <= 0 #consideramos el caso de épsilon, si e´psilon es 0, no hay distancia entre los puntos, sería el mismo, es decir, no hay aproximación pues es el mismo.
        print("Error, el valor de ε es menor que 0")
    end

	#ya que termine, que regrese el último valor calculado
    return xi
end
...
function newtonDN(f, x0, n, h ) #agregamos pedir el argumento h para usar derivada numérica
	#inicializamos un valor que irá variando conforme la iteración
	x= x0 #condición inicial
	for j in 1:n #marcamos el número de iteraciones
		deriv = derivadaNumérica(f,x,h)
		x = x- (f(x0)/deriv) #despejamos de la aproximación de delta, tomando en cuenta que delta= x-x0
	end
	return x
end
...
# ╔═╡ d4ab6bc2-4f8b-4b03-9996-81482f8e0e81
@bind x0s Slider(-10:0.01:10)

# ╔═╡ 49c34b67-c110-45ed-a841-fe06e7929e12
@bind hs Slider(1000:1000:1000000)

# ╔═╡ 666fa778-fbfc-4b1a-b014-a24854b99995
@bind ns Slider(1:10:1000)
...
begin
#editaremos la función newtonDN para ir guardando los 
	function newtonDNplot(f, x0, n, h)
	    x_s = [x0]  # Comenzamos el arreglo donde se irá guardando las primeras entradas de los pares, con los valores iniciales
	    y_s = [f(x0)]  # Comenzamos el arreglo donde se irá guardando las segundas entradas de los pares, con los valores iniciales
	    
	    for j in 1:n #número de aproximaciones
	        deriv = derivadaNumérica(f, x_s[j], h) #obtenemos la derivada
	        x = x_s[j] - f(x_s[j]) / deriv
	        push!(x_s, x)  # lo metemos al final del arreglo donde se almacenan los pares
	        push!(y_s, f(x))  
	    end
	    
	    plot(x_s, y_s, label = "Pares de puntos", marker = :circle, markersize = 4) #graficamos los puntos
	end
	
	f(x) = sin(x)  # Definición de la función f(x)
	
	
	newtonDNplot(f, x0s, ns, hs)
end
