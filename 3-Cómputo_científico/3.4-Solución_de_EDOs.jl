...
# ╔═╡ 65417c0b-bf73-4454-affe-de599d4d958b
tamañoDePaso(5,6,-4)

# ╔═╡ 40f1ca36-c00d-4864-b3f9-c318b45f6b07
tamañoDePaso(5,6,1)
...
function arregloUniforme(t0, tf, N)
    # Vemos si N es positivo y entero
    if N <= 0 
		if isinteger(N)
			error("N no es un entero positivo")
		end
	else
		h = (tf - t0) / (N-1) #h se ajustará a la N puesta
    	numeros = [t0 + i * h for i in 0:N-1] #ponemos hasta N-1 para que no nos de más de N números
		return numeros
	end
end


# ╔═╡ 3ff3a021-11c9-42c9-bc6a-2ea3f53b91af
begin
	
	numeros = arregloUniforme(0, 20, 50)
...
function euler(g, xt0, T)
	T = promote.(T)
    xt = [xt0] #definimos un arreglo donde se almacenarán todas las constantes C que participan en la familia de soluciones, la primera es la proporcionada
	n = length(T) #ya que se hará el procedimiento por cada tiempo donde se evaluará
    
    for i in 2:n #ya tenemos ocupada la primera posición
        ti = T[i-1] #como T es un arreglo y comenzamos desde 2, es posible el i-1, que sería tomar el primer tiempo
        xti = xt[i-1] 
        h = T[i] - T[i-1] #reacomodamos para la función del paso, retomando la anterior en cada iteración
        xtm1 = pasoEuler(ti, xti, g, h)
        push!(xt, xtm1) #hacemos append en el arreglo, hasta el final
    end
...
En ese caso, nuestra EDO se vería de la manera: $x'(t) = g(t)$, entonces al aplicar el método de Euler estaríamos aproximando como: $x_{i+1} = x_i + h \cdot g(t_i)$, podríamos decir que estamos aproximando a una derivada ¿?
...
# ╔═╡ 19100e1b-ff01-410d-8787-9ccab5de7433
begin
	
	g(t) = 2*t
	xt0 = 0
	T = arregloUniforme(0, 1, 10)
