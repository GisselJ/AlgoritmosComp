...
# ╔═╡ 8fa7963b-fa2f-43f1-a8e5-e12af165061a
using Plots
...
# ╔═╡ 2120de7b-64e7-4965-afc5-b22382f24fda
rand()

# ╔═╡ eddda3c9-9238-477e-87c0-e1370708ce27
rand(Int)

# ╔═╡ b86b7c1b-ebea-46e9-ae4b-0bfcf4c38bfd
rand(Bool)

# ╔═╡ 9e22a7c2-8374-4051-8ba2-59865bee00cf
...
# ╔═╡ e9305a3c-b9ac-491b-9084-5fb3a3192b97
rand(n)

# ╔═╡ 21f27506-e1ad-4174-ae7c-f98143f8464d
rand(Int, n)

# ╔═╡ afe05bf9-9b52-4ccf-a04d-2f820d080d99
rand(Bool, n)
...
function aleatorioUniforme(a, b)
	num = a + (b - a) * rand() #como queremos que comience en a el intervalo, lo consideramos como condición inicial y le sumaremos un factor de escala a la función random, con la definición de valor absoluto, a<x<b, despejamos para x y tenemos el factor de escalación
    return num
end


# ╔═╡ 6f432baa-ab44-409c-aab2-b030f31f0027
print(aleatorioUniforme(5,10))
...
# ╔═╡ 05b262c1-bd45-42d6-87c4-be8ac4d66f92
#El histograma debe de tener las barras a la misma altura, o aproximadamente

# ╔═╡ 41a80720-ab01-4931-b86f-1c72cf48c993
function histograma(a, b, i)
    muestra = [aleatorioUniforme(a, b) for j in 1:i] #los límites son a y b, como se definieron en la funcion aleatorio uniforme e i son la cantidad de veces que se usará eésta función, es decir, el tamaño de la muestra del histograma. Creamos un arreglo de los datos
	histogram(muestra) #creamos un histograma aprovechando ésta función d ejulia, donde solo le damos la muestra :)
end

# ╔═╡ 243b354e-afc0-4c4c-a273-e7744839d27d
histograma(0, 50, 1000)
...
function unPaso()
	#como Bool es ya un argumento con 50-50 de probabilidad, basta con if
	if rand(Bool)
		return -1 #no necesita tener relación alguna, pues es random al final 
	else
		return 1
	end
end

# ╔═╡ e4e04615-bbe1-4b06-b50a-ba57dcb84209
unPaso()
...
#podemos hacerlo con un ciclo for, por n veces que vaya agregando el úmero número random al final d enuestro arreglo
function variosPasos(n)
	arreglo =[] #empezamos el arreglo vacío
	for i in 1:n
		val = unPaso()
		push!(arreglo, val)
	end
...
# ╔═╡ ccc9943d-b872-4b2e-ad27-9d09c0d80f7a
variosPasos(5)
...
# ╔═╡ 11ab1e52-d303-44e2-9e36-1eaca1aaf3a2
...
# ╔═╡ 7f06b690-e1ee-4861-86aa-baf7d40902da
...
# ╔═╡ e4ca213b-7388-447c-aaff-bd5312e76fe2
begin
    prev = aleatorioUniforme(1, 25) # para controlar que no nos dé un número enorme de caminatas aleatorias
    n2 = Int(round(prev)) # como aleatorioUniforme produce Floats grandes, redondeamos para después poder convertir a entero
    graficaCaminata(caminataAleatoria(n2))
end
...
function graficaCaminata!(caminata)
    ...
    plot(pasos, caminata, linestyle=:dash, xlabel="Número de pasos", ylabel="Posición", legend=false)

	for _ in 1:5
	
       prev = aleatorioUniforme(1, 25)
       n = Int(round(prev))
       caminata = caminataAleatoria(n)
...
# ╔═╡ 0d26627b-27e1-4e13-bd46-010fe5c820d1
graficaCaminata!(caminataAleatoria(n2))

# ╔═╡ 0e02f8b8-d0d7-494f-837b-2bc224f651fb
...
function caminataAleatoria2D(n)
    ...
    for i in 1:n
        paso_x = round(aleatorioUniforme(-1, 1))
        paso_y = round(aleatorioUniforme(-1, 1))
...
# ╔═╡ 5c0cce6c-d407-4d86-932d-ca0718031e39
caminataAleatoria2D(10)
...
function graficaCaminata2d(caminatax, caminatay)
    
    scatter(caminatax, caminatay, xlabel = "Número de pasos", ylabel = "Posición", legend = false)
end

# ╔═╡ d63169c6-162b-4c47-971f-4729223abede
begin
    prev2d = aleatorioUniforme(1, 25) # para controlar que no nos dé un número enorme de caminatas aleatorias
    n3 = Int(round(prev2d)) # como aleatorioUniforme produce Floats grandes, redondeamos para después poder convertir a entero
	entrada =  caminataAleatoria2D(n3)
    graficaCaminata2d(entrada[1],entrada[2])
end
...
for i in 1:5
    ...
    plot(graficaCaminata2d(caminata[1],caminata[2]))
end
...
# ╔═╡ 830811c9-10ab-481b-9411-beb93fd3781d
function caminataAleatoria3D(n)
    ...
    for i in 1:n
        paso_x = round(aleatorioUniforme(-1, 1))
        paso_y = round(aleatorioUniforme(-1, 1))
        paso_z = round(aleatorioUniforme(-1, 1))
        
        nueva_posicion_x = posiciones_x[end] + paso_x
        nueva_posicion_y = posiciones_y[end] + paso_y
        nueva_posicion_z = posiciones_z[end] + paso_z
        
        push!(posiciones_x, nueva_posicion_x)
        push!(posiciones_y, nueva_posicion_y)
        push!(posiciones_z, nueva_posicion_z)
    end
    
    return [posiciones_x, posiciones_y, posiciones_z]
end

# ╔═╡ e8570903-415b-46be-9593-7338c75b1404
posiciones3d = caminataAleatoria3D(100)

# ╔═╡ 4a520168-7e0d-4424-918a-ce25b4131214
plot3d(posiciones3d[1], posiciones3d[2], posiciones3d[3], xlabel=" X", ylabel=" Y", zlabel=" Z")
